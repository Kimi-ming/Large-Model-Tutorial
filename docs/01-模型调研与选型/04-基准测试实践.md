...
本章节将带您动手实践模型测试，学习如何客观评估不同模型的性能。
...
- 熟悉Python编程
- 了解基本的深度学习概念
- 已安装教程环境（参考：[环境安装指南](../05-使用说明/01-环境安装指南.md)）
...
| 模型 | 任务类型 | 预期推理速度 | 预期显存 |
|------|---------|-------------|----------|
| **CLIP-ViT-B/32** | 图文检索 | ~50 images/sec | 2.5GB |
| **SAM-ViT-B** | 图像分割 | ~20 images/sec | 4GB |
| **BLIP-2-OPT-2.7B** | 图像描述 | ~8 images/sec | 6.8GB |

> 💡 **说明**：这些模型覆盖了视觉大模型的3个主要方向，且资源需求适中（≤8GB显存），适合大多数学习者测试。

### 评测指标

我们将测试以下指标：

1. **性能指标**：准确率、召回率、F1-score（任务相关）
2. **效率指标**：推理速度、吞吐量、延迟
3. **资源指标**：显存占用、模型大小、CPU/GPU利用率
...
> 💡 **提示**：测试数据仅用于benchmark，不需要标注，任意图像即可。
...
创建 `code/01-model-evaluation/benchmark/speed_test.py`：
...
创建 `code/01-model-evaluation/benchmark/memory_test.py`：
...
创建 `code/01-model-evaluation/benchmark/accuracy_test.py`：
...
创建 `code/01-model-evaluation/benchmark/visualize_results.py`：
...
创建 `code/01-model-evaluation/benchmark/generate_report.py`：
...
2. **进阶任务**（选做）：
   - 对比2-3个不同模型的性能
   - 测试不同batch size和量化精度的影响
   - 编写自定义的准确率测试脚本

3. **高级任务**（挑战）：
   - 在不同硬件上重复测试（如CPU、不同GPU）
   - 实现自动化的CI测试流程
   - 对比开源模型与商业API的性能/成本
...
- [../02-模型微调技术/](../02-模型微调技术/) - 学习如何微调模型
- [../03-数据集准备/](../03-数据集准备/) - 准备高质量训练数据

---

## 📚 参考资源

- [PyTorch Benchmark工具](https://pytorch.org/tutorials/recipes/recipes/benchmark.html)
- [HuggingFace性能优化指南](https://huggingface.co/docs/transformers/performance)
- [NVIDIA Nsight Systems](https://developer.nvidia.com/nsight-systems) - 高级性能分析工具
