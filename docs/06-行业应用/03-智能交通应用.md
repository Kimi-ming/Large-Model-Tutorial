...
> **注意**：当前实现为简化版零样本分类，检测整个图像的车辆类型。
> - `bbox`：覆盖全图（实际应用中需配合目标检测模型精确定位）
> - `position`：固定为"center"（未实现车道检测）
> - 不包含`color`字段（需额外的颜色识别模型）
> 
> 完整的车辆检测（精确定位+属性识别）建议使用YOLOv8等目标检测模型。
...
```json
{
  ...
  "scene_description": "This is a traffic accident scene showing two vehicles involved in a rear-end collision...",
  "possible_cause": "following too closely or sudden braking",
  "recommendation": "清理现场，快速理赔"
}
```

> **注意**：当前实现的字段说明
> - `damage_level`：统一使用事故整体的严重程度（`low`/`medium`/`high`）
> - `damage_area`：简化为`front`（前方车辆）或`rear`（后方车辆）
> - `accident_type`、`possible_cause`：由BLIP-2生成，可能为英文
> - `scene_description`：详细场景描述，通常较长
> 
> 完整的车辆损伤评估（如具体部位、严重程度分级）需要专门的损伤识别模型。
