...
# Python API方式
# 压线检测
lane_violation = violation_detector.detect_lane_violation(
    image=image,
    lane_lines='auto'  # 自动检测车道线，或手动指定
)
...
import sys
sys.path.insert(0, 'code/05-applications/traffic')
exec(open('code/05-applications/traffic/traffic_analyzer.py').read(), globals())
...
count_result = traffic_analyzer.count_vehicles_from_video(
    ...
    counting_line=[[0, 400], [1920, 400]],  # 计数线
    direction='both'  # 双向计数
)

print(f"总车辆数: {count_result['total_count']}")
print(f"车型分布:")
for vehicle_type, count in count_result['by_type'].items():
...
print(f"拥堵等级: {congestion['level']}")  # smooth/moderate/congested/severe
print(f"车辆密度: {congestion['density']:.1f} 车/百米")
print(f"平均车速: {congestion['avg_speed']:.1f} km/h")
```
